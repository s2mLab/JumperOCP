...
def prepare_ocp(model_path, phase_time, ns, time_min, time_max):
    ...
    objective_functions.add(Objective.Mayer.MINIMIZE_PREDICTED_COM_HEIGHT, weight=-100, phase=0)
    ...
    constraints.add(Constraint.TRACK_STATE, phase=0, node=Node.END, index=3, min_bound=1.0, max_bound=np.inf)
    ...
    u_bounds = BoundsList()
    u_bounds.add([[-500] * tau_mapping.reduce.len, [500] * tau_mapping.reduce.len])
