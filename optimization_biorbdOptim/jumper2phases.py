...
def prepare_ocp(model_path, phase_time, number_shooting_points, use_symmetry=True):
    ...
    # )
    problem_type = (
        {"type": ProblemType.TORQUE_DRIVEN_WITH_CONTACT},
        {"type": ProblemType.TORQUE_DRIVEN_WITH_CONTACT},
    )
    ...
    U_bounds = [
        Bounds(
            ...
            min_bound=[torque_min] * tau_m.reduce.len, max_bound=[torque_max] * tau_m.reduce.len
        )
...
def run_and_save_ocp(model_path, phase_time, number_shooting_points):
    ...
    sol = ocp.solve(options_ipopt={"hessian_approximation": "limited-memory", "max_iter": 1000}, show_online_optim=True)
